...
* **AI-Likeness Screen (Stage 2B.5):** `papernews/ai_detect.py` runs the lyc8503/AITextDetector architecture (char n-gram TF-IDF + linear SVM) with an English-trained model, evaluated in pure Python from the vendored artifact `papernews/ai_model.json.gz` (override: `PAPERNEWS_AI_MODEL`). Training/eval live in `scripts/train_ai_classifier.py` / `scripts/eval_ai_detect.py` (scikit-learn is dev-only; runtime inference must stay dependency-free and pinned to sklearn by the parity test). The screen deranks flagged articles below the category-budget cut (optional hard drop via `ai_drop_threshold`); without an artifact it observes only — never reintroduce hand-tuned scoring rules. It is a noise dial, not a detector: unreliable (short) samples are never penalized. Metrics ride `RawDocument.ai_metrics` → `ArticleChunk.ai_metrics` and surface as a per-article stylometrics footer plus funnel counts in the finished paper.
