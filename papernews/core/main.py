...
def triage_process_a_filter(documents: List[RawDocument], prefs: dict) -> List[RawDocument]:
    filtered_docs = []
...
def triage_process_b_rank(documents: List[RawDocument], prefs: dict) -> List[RawDocument]:
...
def triage_process_c_budget(documents: List[RawDocument], limits: dict, prefs: dict) -> List[RawDocument]:
    """
...
def stage3_hybrid_construction(documents: List[RawDocument], prefs: dict) -> tuple[List[ArticleChunk], Telemetry]:
    processed_chunks = []
    ...
    for doc in documents:
        ...
        try:
            domain_source = urlparse(doc.metadata.get("feed_url", doc.source_id)).netloc.replace("www.", "")
        except:
            pass
...
def stage5_bespoke_render(legacy_articles: List[dict], total_telemetry: Telemetry) -> Path:
    ...
    decorations = {
        ...
        "quote": {"text": "Benjamin you stop pickin' the bark off of that tree!", "author": "Grandma Brandt"},
        "world_news": [], 
        "dyk": []
    }
    
    pdf_path = build_pdf(date=today_str, articles=legacy_articles, out_dir=out_dir, decorations=decorations)
    return pdf_path
...
if __name__ == "__main__":
    # Ensure this runs from the root of the project where sources.toml lives
