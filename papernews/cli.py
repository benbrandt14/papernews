        ...
        for it in items:
            ...
            if store.exists(it.url, it.title):
                if force:
                    ...
                    store.con.execute(
                        "DELETE FROM article WHERE url_hash = ?", (url_hash,)
                    )
                    store.con.commit()
                else:
                    store.insert_raw(
...
def cmd_select(store: Store, sources: list[dict], prefs: dict, cat_limits: dict) -> int:
    ...
    for cat in categories:
        ...
        _log(
            f"[select] Category '{cat}': evaluating {len(pending)} pending (limit {limit})"
        )
...
def cmd_summarize(store: Store, workers: int) -> int:
    ...
    _log(
        f"[summarize] {len(pending)} pending in {len(batches)} batch(es) "
    ...
    def _run_batch(rows: list) -> list[tuple[str, str]]:
        ...
        if len(out) != len(rows):
            raise ValueError(
                f"LLM returned {len(out)} items, expected {len(rows)}. Aborting batch."
            )
...
def cmd_rewrite(store: Store, workers: int) -> int:
    ...
    batches = _chunks(pending, _BATCH_SIZE)
    _log(
        f"[rewrite] {len(pending)} pending in {len(batches)} batch(es) "
        f"of {_BATCH_SIZE} (workers={workers})"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Dev utilities
    sp_cl = sub.add_parser(
        "clean", help="clean localized compile caches and storage indices"
    )
    sp_cl.add_argument(
        "--db",
        action="store_true",
        help="wipe database clean to force full structural refetching",
    )

    sp_gat = sub.add_parser(
        "gather", help="fetch + extract new articles into the store"
    )
    sp_gat.add_argument(
        "--force",
        action="store_true",
        help="evict and re-extract existing matching records",
    )
    ...
    sp_rw = sub.add_parser(
        "rewrite", help="reformat article bodies into clean paragraphs"
    )
    sp_rw.add_argument("--workers", type=int, default=6)

    sp_ing = sub.add_parser(
        "ingest", help="gather + select + summarize + rewrite (no PDF)"
    )
    sp_ing.add_argument("--workers", type=int, default=6)
    sp_ing.add_argument(
        "--force", action="store_true", help="force raw collection overrides"
    )
    sp_ing.add_argument(
        "--skip-rewrite",
        action="store_true",
        help="bypass expensive context text structural rewrites",
    )
    ...
    source_category_map = {
        s["name"]: s.get("category", "Uncategorized") for s in sources
    }
