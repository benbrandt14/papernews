...
from papernews.core.main import triage_process_a_filter, triage_process_b_rank, triage_process_c_budget
...
def test_budget_enforces_category_limits():
    ...
    docs = [
        RawDocument(source_id="1", content_type="rss", raw_text="A", metadata={"category": "Tech"}),
        RawDocument(source_id="2", content_type="rss", raw_text="A", metadata={"category": "Tech"}),
        RawDocument(source_id="3", content_type="rss", raw_text="A", metadata={"category": "Tech"}),
        RawDocument(source_id="4", content_type="rss", raw_text="A", metadata={"category": "Science"}),
        RawDocument(source_id="5", content_type="rss", raw_text="A", metadata={"category": "Science"}),
    ]
...
def test_budget_uses_default_limit():
    ...
    prefs = {"default_category_limit": 1}
    docs = [
        RawDocument(source_id="1", content_type="rss", raw_text="A", metadata={"category": "Misc"}),
        RawDocument(source_id="2", content_type="rss", raw_text="A", metadata={"category": "Misc"}),
    ]
    result = func(docs, limits, prefs)
    assert len(result) == 1
