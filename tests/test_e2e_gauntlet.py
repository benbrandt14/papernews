...
def test_e2e_gauntlet(mocker, tmp_path, monkeypatch):
    ...
    mocker.patch(
        "papernews.core.main.llm_select_article",
        side_effect=lambda doc, prefs: (True, Telemetry()),
    )
    mocker.patch(
        "papernews.core.main.llm_summarize_article",
        side_effect=lambda doc: ("Mock summary", Telemetry()),
    )
    mocker.patch(
        "papernews.core.main.llm_format_body",
        side_effect=lambda doc: (doc.raw_text, Telemetry()),
    )
    ...
    typst_path = (
        tmp_path / "output" / ".build" / f"{date.today().strftime('%Y-%m-%d')}.typ"
    )
    assert typst_path.exists()

    content = typst_path.read_text(encoding="utf-8")

    assert "\\@hello \\#world \\$money" in content, (
        "Control characters were not properly escaped!"
    )
    ...
    assert check_balanced_delimiters(content), (
        "Found unbalanced [], (), or {} in Typst output!"
    )
