...
class StubEntry:
    ...
    def get(self, key, default=""):
        if key == "title": return self.title_attr
        if key == "published": return self.published_attr
        if key == "updated": return self.updated_attr
        return default
...
def ingestion_plugin(request, mocker):
    ...
    if name == "hn_plugin":
        ...
        class FakeResponse:
            ...
            def json(self): return {"hits": [{"title": "t", "url": "http://example.com", "objectID": "1", "points": 100}]}
...
def test_plugin_network_failure(ingestion_plugin, mocker):
    ...
    assert len(docs) == 0 # because fetch failed, it should continue and return empty or drop
...
def test_plugin_successful_ingestion(ingestion_plugin, mocker):
    ...
    mock_extract = mocker.patch("trafilatura.extract", return_value="Here is some extracted text that is long enough to bypass limits " * 20)
