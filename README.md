...
2. **Stage 2: Filtering** — Enforces deterministic category limits, local ranking heuristics, and regex blacklists natively in Python (zero API cost). Includes the **AI-likeness screen** (`papernews/ai_detect.py`): the [lyc8503/AITextDetector](https://github.com/lyc8503/AITextDetector) architecture — a linear SVM over character n-gram TF-IDF — trained on labeled *English* human/LLM text (the upstream model is Chinese-only) and evaluated in pure Python from a small vendored artifact, so production needs no ML runtime. The classifier's calibrated `ai_likelihood` deranks machine-flavored articles below the category-budget cut line before they cost LLM tokens; it's a configurable noise dial, not a detector — short samples are never penalized and false negatives are fine by design. Train the artifact once with `uv run --with scikit-learn python scripts/train_ai_classifier.py` (HC3-English by default; add a [ghostbuster-data](https://github.com/vivek3141/ghostbuster-data) checkout with `--ghostbuster` for news-domain coverage) and commit `papernews/ai_model.json.gz`; held-out ROC-AUC/accuracy are printed and stored in the artifact, and `scripts/eval_ai_detect.py` measures any artifact against any corpus. Without an artifact the screen only observes. Tune it in `[preferences]` (`ai_detection_enabled`, `ai_derank_threshold`, `ai_derank_penalty`, optional `ai_drop_threshold`). Each typeset article discloses its score, descriptive stylometrics (sentence-length burstiness, lexical diversity), and the model id in a small footer, and the front-matter funnel reports how many articles the screen deranked or dropped.
