...
2. **Stage 2: Filtering** — Enforces deterministic category limits, local ranking heuristics, and regex blacklists natively in Python (zero API cost). Includes the **AI-likeness screen** (`papernews/ai_detect.py`, adapted from [lyc8503/AITextDetector](https://github.com/lyc8503/AITextDetector)): article-level stylometrics — sentence-length burstiness, moving-window lexical diversity, and a stock-LLM-phrase lexicon — combine into an `ai_likelihood` score that deranks formulaic articles below the category-budget cut line before they cost LLM tokens. It's a configurable noise dial, not a detector: short samples are never penalized, and false negatives are fine by design. Tune it in `[preferences]` (`ai_detection_enabled`, `ai_derank_threshold`, `ai_derank_penalty`, and optional `ai_drop_threshold` for a hard drop). Each typeset article discloses its metrics in a small stylometrics footer, and the front-matter funnel reports how many articles the screen deranked or dropped.
